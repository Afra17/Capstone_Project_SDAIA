...
16. مواءمة المشروع مع رؤية المملكة 2030
...
يتم [Vendor B] حالاً مؤسسياً عملياً لتطوير وتفعيل مساعد ذكي داخلي يحقق "مصدر الحقيقة الواحد" عبر أتمتة جمع البيانات (End-to-End)،
وتوحيدها والتحقق من جودتها،
...
- • تميّز [Vendor B] التنافسي يتمثل في:
- • واقعية التنفيذ: تحويل النطاق إلى تدفقات تشغيل،
عقود تَكامل(Contracts)،
قواعد جودة بيانات،
...
1. بناء طبقة استيعاب (Data Ingestion) تدعم (API/DB Connectors/CSV-Excel/File):
 Landing Zone/PDF-Word extraction عند الحاجة.

## 3

-
2. تطبيق قواعد جودة (Validation + Standardization + Deduplication + Mapping) مع سجل أخطاء(Data Quality Log).
- 3. جدول تشغيل (Schedulers) وإدارة الاعتمادية بين المهام وإعادة المحاولة،
والتنبيه.
...
كيف:
١. تصميم نموذج بيانات موحد (Canonical Data Model) + طبقة Semantic KPIs.
٢. مستودع تحليلي (Data Warehouse/Lakehouse حسب بنية الجهة) لاستعلامات سريعة.
٣. لوحات تنفيذية بمرشحات (Sector/Agency/Initiative/Time) وتصدير.

الناتج: مصدر حقيقة واحد لتقارير قابلة للتصدير.

معيار قبول/قياس: لوحات KPIs مع اعتماد قاموس بيانات رسمي،
وزمن تحميل للوحة القيادية ≥ ٥ ثوانٍ لسيناريو لوحات الاستعلام القياسية.
...
3. محرك Exception Detection: عتبات،
قواعد عمل Anomaly detection + ربطها بالتنبيهات.

النتائج: تنبيهات مبكرة + توقعات قابلة للتفسير.
معيار قبول/قياس: تغطية ١٠٠٪ من KPIs المعتمدة بحسابات وصفية،
...
1. تطبيق (RAG): Retriever (بيانات المستودع + وثائق تعريفية) ثم Generator ينتج إجابة “مؤسسة” بمراجع داخلي (IDs/Query results).
 
2. طبقة NLP اللغة العربية والإنجليزية وربط الكيانات (Initiative/KPI/Owner/Date).
 
3. تحسين الاستدلال (Inference Optimization): Cache + Context management + Answerability checks.

- النتائج: إجابات قابلة للتدقيق مع مؤشرات “مرجعية” داخلية.
- معيار قبول/قياس: 90% من أسئلة UAT القياسية يُجاب بإجابات صحيحة قابلة للتتبع،
...
- • ماذا: إنتاج جداول/رسوم/نقاط تنفيذية يمكن إرجاعها في تقارير القيادة.
...
- “اعرض المبادرات المتأخرة أكثر من 14 يوماً في قطاع X خلال 30 يوماً مع أسباب التأخر الأعلى تكراراً.”
- “ما KPIs التي لديها انحراف سلبي متكرر ثلاث دورات قياس؟
وما المبادرات المرتبطة بها؟”
- “ما أعلى 10 مبادرات في عالية الأثر من حيث مؤشر المخاطر المركب؟
وما عوامل المخاطر؟”
...
- • منصات/ERP مالية/موارد (مثل Oracle/SAP حسب المتاح).
...
- • تتبع الصحيح وقياس عدم اليقين (Confidence bands) لمنع قرارات خاطئة.
...
- • معيار قبول: قبول UAT للوحـات عبر سيناريوهات اختبار مـعرّفة وموثّقة.
...
- • النطاق: يتناول بروحات قيادية (Executive)،
قطاع/إدارة،
مبادرة،
Chat،
KPI،
...
- • Metadata Catalog لتعريف المصادر و الحقول و التحولات.
...
- اختيار نماذج وفق حالة الاستخدام:
 ...
 ◦ Time-series التوقعات ربع/شهري حسب دورية القياس
...
- ٥.٢ طبقة منطق الأعمال والحوكمة (Business Logic Layer)
- • محرك قواعد الأعمال: عتبات/انحراف/تصعيد/إشعارات/قاموس أسباب التغير.
- • IAM/RBAC أو سياسات التصيدير والمشاركة.
...
- • واجهة ويب داخلية آمنة: لوحات تفاعلية + صفحات مباشرة + KPI/مركز تقارير + واجهة محادثة.
...
**الخـيارات المدعـومة:** داخلي/سحابة خاصة/سحابة عامة داخل المملكة حسب قرار الجهة.
**تصميم البنية:**
...
5. إعداد CI/CD ومراقبة وتشغيل (Observability).
...
٤،
٧،
٢ المرحلة ٤: الاختبارات وضمان الجودة(QA & Testing)
...
هـ ٢, ٧،
المرحلة ٥: النشر والإطلاق (Deployment & Go-Live)
...
٧،
٢،
٢ المرحلة ٧: التشغيل والدعم ٣ – (Operations & Support) أشهر بعد النشر
...
قبول: اختيار صلاحيات مثبت منع الوصول غير المصرح.
...
مع مالك لكل خطر وخطة تخفيف ومؤشر إنذار مبكر.
...
- • تخفيف،
تثبيت P1 مبكرًا + Contracts + بيئة تكامل + اختبار مبكر / مؤشر: فشل تكامل متكرر 3 مرات أسبوعيًا.
...
ويقر [Vendor B] أن أي نقص في المستندات التنظيمية أو الشهادات المطلوبة قد يترتب عليه استبعاد العرض وفق ما تحدده الجهة ولجنتها المختصة.
...
(أ) استكمال الشهادات/المتطلبات المطلوبة خلال ١٠ أيام عمل من تاريخ الإشعار في حال كانت ناقصة،
وإلا يحق للجهة استبعاد العرض أو إنهاء العقد طبقاً لما يطبق بالضمان الابتدائي وفق وثائق المنافسة.
(ب) الإفصاح عن بند سعر صرف العملة الأجنبية أو اعتباره معملًا على إجمالي العرض وفق ما تقرره لجنة فحص العروض.
(ج) في حال تنفيذ البنود المسعرة على إجمالي العرض،
يتم تنفيذها على حساب [Vendor B] ثم يُقسم متوسط السعر وفق آلية التسعير المعتمدة.
(د) الإفصاح عن البنود غير المنطبقة إذا تبين أنها لا تمثل السعر الحقيقي،
...
فالحجة تُرجح المنافسة بين العروض المتساوية إذا سمحت الشروط بذلك.
